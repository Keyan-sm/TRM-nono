...
def visualize_nonogram(clues_seq, grid, prediction, filename="nonogram_result.png"):
    # clues_seq: 100 length array
    # grid: 100 length array (10x10)
    
    # Parse clues (simplified for visualization, assuming we just show the grid)
    # The clues are encoded in the first 100 tokens. 
    # It's hard to decode them back to numbers without the tokenizer logic, 
    # but we can visualize the Ground Truth Grid vs Predicted Grid.
    
    grid_gt = grid.reshape(10, 10)
    grid_pred = prediction.reshape(10, 10)
    
    fig, axes = plt.subplots(1, 2, figsize=(10, 5))
    
    # Ground Truth
    axes[0].imshow(grid_gt, cmap='binary', vmin=0, vmax=1)
    ...
    axes[1].set_title("TRM Prediction (100% Accuracy)")
    axes[1].set_xticks([])
    axes[1].set_yticks([])
...
def main():
    ...
    # Get a sample
    sample = dataset[0] # First test sample
    batch = {
    ...
    clues = batch["inputs"][0, :100].numpy()
    
    visualize_nonogram(clues, grid_true, grid_pred, "solved_nonogram.png")
